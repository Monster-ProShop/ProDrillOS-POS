...
export default function Layout({ children }: { children: React.ReactNode }) {
  const logo = process.env.NEXT_PUBLIC_BRAND_LOGO || "/brand/prodrillos-logo.png";
    ...
    <header className="border-b border-white/10 px-5 py-5 md:px-10">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-5">
        <a href="/pos" aria-label="ProDrillOS home" className="flex items-center gap-3">
          <img src={logo} alt="" className="h-12 w-12 rounded-lg object-cover" /><span className="text-2xl font-bold tracking-tight">ProDrill<span className="text-brand">OS</span></span>
